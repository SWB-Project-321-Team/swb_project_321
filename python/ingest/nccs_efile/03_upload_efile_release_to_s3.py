"""
Step 03: Upload the local NCCS efile raw assets and metadata to S3.
"""
...
from common import (
    DEFAULT_S3_BUCKET,
    DEFAULT_S3_REGION,
    EFILE_CATALOG_SNAPSHOT,
    EFILE_DATASET_SNAPSHOT,
    EFILE_RAW_DIR,
    LATEST_RELEASE_JSON,
    META_DIR,
    META_PREFIX,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Upload the local NCCS efile raw assets and metadata to S3.")
    ...
    banner("STEP 03 - UPLOAD NCCS EFILE RAW ASSETS TO S3")
    load_env_from_secrets()
    ...
    dataset_snapshot_path = args.metadata_dir / EFILE_DATASET_SNAPSHOT.name
    catalog_snapshot_path = args.metadata_dir / EFILE_CATALOG_SNAPSHOT.name
