...
Run from repo root: python python/ingest/givingtuesday_990/api/01_fetch_zip_to_county.py
...
then run with --local path/to/downloaded.csv to normalize and copy to reference.
"""
...
_PYTHON_DIR = _SCRIPT_DIR.parents[3]  # api -> givingtuesday_990 -> ingest -> python
...
def _download_census() -> pd.DataFrame:
    """Download Census ZCTA–county file and return DataFrame with ZIP, FIPS, area."""
...
def _census_to_zip_fips(df: pd.DataFrame) -> pd.DataFrame:
    """Convert Census ZCTA–county table to one row per ZIP with FIPS (largest overlap)."""
...
def main() -> None:
    ...
    if args.source == "local" or args.local is not None:
        ...
        print(f"Normalizing local file: {path}")
        out = _normalize_local_csv(path)
    else:
        print(f"Downloading Census ZCTA–county from {CENSUS_ZCTA_COUNTY_URL}...")
        df = _download_census()
    ...
    print(f"Wrote {len(out)} rows to {ZIP_TO_COUNTY_CSV}")
