...
steps in docs/data_sources/irs_990/990_data_fetch_plan.md (run 01_, 02_, 03_ in python/ingest/givingtuesday_990/api/).
...
Run from repo root: python python/ingest/givingtuesday_990/api/04_fetch_givingtuesday_990.py
For immediate output in terminal/IDE, use: python -u python/ingest/givingtuesday_990/api/04_fetch_givingtuesday_990.py
"""
...
def _build_ein_list_from_bmf() -> pd.DataFrame:
    ...
    if not REF_GEOID_XLSX.exists():
        raise FileNotFoundError(
            f"EIN list not found at {EIN_LIST_CSV}. Create it per docs/data_sources/irs_990/990_data_fetch_plan.md, "
