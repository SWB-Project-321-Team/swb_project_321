"""
Step 04: Verify source/local/S3 byte parity for the raw NCCS e-Postcard assets.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Verify raw NCCS e-Postcard files against source and S3 sizes.")
    ...
    banner("STEP 04 - VERIFY SOURCE / LOCAL / S3 E-POSTCARD SIZE PARITY")
    load_env_from_secrets()
    ensure_work_dirs(postcard_raw_dir=args.postcard_raw_dir, metadata_dir=args.metadata_dir)

    print(f"[verify] Requested snapshot year: {args.snapshot_year}", flush=True)
    print(f"[verify] Requested snapshot months: {args.snapshot_months}", flush=True)
    ...
    report_path = size_report_path(args.metadata_dir, snapshot_year)
