# irs_990 — IRS Form 990 TEOS XML pipeline (historical S3 target)

> **Infrastructure status (2026-07-23):** the documented project bucket was intentionally deleted. This streaming IRS-to-S3 workflow is inactive and must not recreate `swb-321-irs990-teos`; the documentation is retained for lineage and possible redesign to a separately approved destination.

These retained scripts were designed to pull **Form 990 series (e-file) XML** from IRS TEOS and stream them directly to the former project bucket. That IRS-to-S3 path is now inactive; the code is retained as historical pipeline implementation.

**Bucket:** `swb-321-irs990-teos`  
**Prefix:** `bronze/irs990/teos_xml`  
**Years:** 2021 through current year (configurable).

See **docs/data_sources/irs_990/irs_990_teos_s3_plan.md** for the full plan and S3 layout.
...
0. **00_fetch_bmf.py** — (Optional compatibility wrapper.) Delegates to the canonical `python/ingest/irs_bmf/01_fetch_bmf_release.py` pipeline step and can also trigger the raw upload step. Use this if older commands still point at `irs_990`.
...
2. **irs_990**: Run **01** (index), **02** (uploads all ZIP parts), **03** (parses and keeps only rows whose org ZIP → county is in GEOID reference; sets `region` from the reference). **04** (merge) uses the same GEOID reference + zip_to_county when merging from silver parts.
...
```bash
python python/ingest/irs_990/00_fetch_bmf.py
python python/ingest/irs_990/00_fetch_bmf.py --states sd mn mt az
```
Writes to `01_data/raw/irs_bmf/eo_{state}.csv`. Default states: sd, mn, mt, az.

**Fetch BMF and upload to S3 (same bucket as TEOS):**
```bash
python python/ingest/irs_990/00_fetch_bmf.py --upload
```
...
```bash
python python/ingest/irs_990/00b_filter_bmf_to_benchmark_upload_silver.py
python python/ingest/irs_990/00b_filter_bmf_to_benchmark_upload_silver.py --no-upload
```
...
```bash
python python/ingest/irs_990/build_ein_list.py
```
...
**Custom year range:**
```bash
python python/ingest/irs_990/02_upload_irs_990_zips_to_s3.py --start-year 2022 --end-year 2025
```

**Verify S3 ZIPs match IRS source (size, or size + MD5):**
```bash
python scripts/diagnostics/irs_990/verify_irs_vs_s3.py --year 2021
python scripts/diagnostics/irs_990/verify_irs_vs_s3.py --year 2021 --zip 2021_TEOS_XML_01A.zip --hash
```
...
```bash
python python/ingest/irs_990/01_upload_irs_990_index_to_s3.py --bucket my-bucket --prefix bronze/irs990 --region us-east-2
```
...
```bash
python python/ingest/irs_990/03_parse_irs_990_zips_to_staging.py --start-year 2022 --end-year 2024 --output path/to/filings.parquet
```
...
```bash
python python/ingest/irs_990/03_parse_irs_990_zips_to_staging.py --workers 4
python python/ingest/irs_990/03_parse_irs_990_zips_to_staging.py --prefetch --download-workers 4 --in-memory-mb 200
```
...
```bash
python python/ingest/irs_990/03_parse_irs_990_zips_to_staging.py --skip-processed
```
...
```bash
python python/ingest/irs_990/03_parse_irs_990_zips_to_staging.py --output-full 01_data/staging/filing/irs_990_filings_full.parquet
```
...
python python/ingest/irs_990/03_parse_irs_990_zips_to_staging.py --output-parts-dir 01_data/staging/filing/parts --output 01_data/staging/filing/irs_990_filings.parquet

# Later: merge from silver only, no re-parse
python python/ingest/irs_990/04_merge_990_parts_to_staging.py --parts-dir 01_data/staging/filing/parts --output 01_data/staging/filing/irs_990_filings.parquet
```
...
- Minimal storage and no DB server: staging stays as Parquet in OneDrive; DuckDB can query Parquet directly (see **sql/README.md** and **docs/infrastructure/onedrive/structure.md**).
