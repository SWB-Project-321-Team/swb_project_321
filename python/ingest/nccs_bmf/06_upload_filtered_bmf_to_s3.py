"""
Step 06: Upload the filtered NCCS BMF yearly benchmark outputs to S3.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Upload the filtered NCCS BMF yearly benchmark outputs to S3.")
    ...
    banner("STEP 06 - UPLOAD FILTERED NCCS BMF OUTPUTS TO S3")
    load_env_from_secrets()
    ...
    manifest_key = f"{args.silver_prefix.rstrip('/')}/metadata/{manifest_path.name}"
