...
> **Historical snapshot:** the bucket was intentionally deleted by 2026-07-23. This tree describes its 2026-04-11 state and must not be used to recreate the bucket.
...
- Older object-level table: `docs/infrastructure/s3/bucket_inventory_2026-03-22.md` (snapshot `2026-03-22`, 298 objects; superseded for counts).
...
- BLS raw upload: `python/ingest/BLS/fetch_bls.py` (`bronze/bls/…`); Silver BLS paths are not defined in that script (search repo for `silver/bls`).
- IRS BMF keys: `python/ingest/irs_bmf/common.py`
- Giving Tuesday: `python/ingest/givingtuesday_990/datamart/common.py`
- Combined: `python/ingest/combined_990/common.py` (`SILVER_PREFIX`, `METADATA_S3_PREFIX`)
- ACS bronze uploads: `python/ingest/acs/acs_api.py`. Silver ACS / Silver BLS / USAspending S3 keys are not centralized in `common.py`; see `docs/data_sources/usaspending.md` and repo search for `usaspending`.
