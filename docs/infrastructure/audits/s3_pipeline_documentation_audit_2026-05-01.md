...
> **Historical audit:** this records the bucket state observed on 2026-05-01. The project bucket was intentionally deleted by 2026-07-23, so none of the inventory counts below describe an active service and the bucket must not be recreated from this audit.

## Current Disposition (2026-07-23)

- Inventory drift was superseded by the final confirmed 2026-05-02 snapshot; the bucket was later intentionally deleted.
- Current local pipeline references include the literal public-source URLs identified as missing in F-002.
- Current Markdown uses portable repository-relative paths. Machine-local absolute paths remain only inside retained JSON/CSV metadata as historical run provenance.
- Coverage evidence is now retained locally under `docs/deliverables/preprocessing/technical_reference/quality/coverage_evidence/`.

The remainder of this file preserves the evidence and conclusions as they existed on the audit date.

Audit date: 2026-05-01

Bucket audited: `s3://swb-321-irs990-teos/`

Supporting inventory: `docs/infrastructure/audits/s3_object_inventory_classification_2026-05-01.csv`
...
1. At audit time, the root S3 inventory docs were stale: the 2026-05-01 live inventory was 378 objects and 35,690,814,575 bytes, while `docs/infrastructure/s3/bucket_tree_2026-04-11.md` recorded 373 objects and `docs/infrastructure/s3/bucket_inventory_2026-03-22.md` recorded 298 objects.
...
At audit time, all 12 target Markdown docs published to S3 were byte-identical to their local copies in the repository’s earlier documentation layout. Current local copies are organized under `docs/deliverables/preprocessing/`; the six coverage CSVs are now retained under `technical_reference/quality/coverage_evidence/`.
...
| --- | --- | --- | --- |
| `docs/infrastructure/s3/bucket_tree_2026-04-11.md` | Live verification on 2026-04-11: 373 objects | 2026-05-01 live listing: 378 objects | Stale by 5 objects |
| `docs/infrastructure/s3/bucket_inventory_2026-03-22.md` | Historical snapshot on 2026-03-22: 298 objects | 2026-05-01 live listing: 378 objects | Correctly labeled historical, but no longer usable for current counts |

The classified object-level inventory is stored in `docs/infrastructure/audits/s3_object_inventory_classification_2026-05-01.csv`.
...
No mismatch was found between the reviewed extraction code and the published variable mapping docs for target families. At audit time, the S3 coverage CSVs were generated run artifacts without tracked local copies. They are now retained locally under `docs/deliverables/preprocessing/technical_reference/quality/coverage_evidence/`.
...
| --- | --- | --- | --- | --- | --- |
| F-001 | P2 | Bucket inventory docs were stale at audit time. | The 2026-05-01 S3 listing had 378 objects and 35,690,814,575 bytes. `docs/infrastructure/s3/bucket_tree_2026-04-11.md` recorded 373 objects, while `docs/infrastructure/s3/bucket_inventory_2026-03-22.md` recorded 298 objects. | `docs/infrastructure/s3/bucket_tree_2026-04-11.md`; `docs/infrastructure/s3/bucket_inventory_2026-03-22.md` | Resolved historically by the later 2026-05-02 snapshot. The bucket has since been intentionally deleted; retain all snapshots as dated lineage only. |
...
| F-004 | Info | Coverage CSVs were S3-generated artifacts without tracked local copies at audit time. | Six S3 coverage CSVs were present and inspected. | `silver/*/analysis/metadata/*_analysis_variable_coverage.csv` | Resolved for repository access: current copies are retained under `docs/deliverables/preprocessing/technical_reference/quality/coverage_evidence/`; former S3 keys remain historical lineage. |
...
| Criterion | Status |
| --- | --- |
| Fresh S3 inventory reconciled target raw, silver, analysis, metadata, and documentation objects at audit time | Met historically; 378 objects were inventoried and classified in the supporting CSV |
