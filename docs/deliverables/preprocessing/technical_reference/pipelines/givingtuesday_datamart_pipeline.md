...
> **Infrastructure status (2026-07-23):** the pipeline logic and retained artifacts remain documented, but the former project S3 bucket was intentionally deleted. S3 publication and parity-check steps in this reference are historical and must not recreate `swb-321-irs990-teos`.
...
It reflects the code as it exists now under `python/ingest/givingtuesday_990/datamart/`, including the current benchmark tax-year window of `2022-2024`, the current ROI-first mixed-file build in step 07, the current GT final analysis extract in step 13, and the current GT analysis-upload step in step 14.
...
- Data producer: GivingTuesday publishes the DataMart catalog and raw DataMart files used by this pipeline.
- Collection method: the pipeline consumes GivingTuesday DataMart aggregates and field extracts derived from public nonprofit filing data; it is not a voluntary survey collection performed by this repo.
- Inclusion criteria and limitations: the pipeline selects the required Combined Forms DataMart plus Basic Fields files for Forms 990, 990-EZ, and 990-PF, restricts the current analysis window to `2022-2024`, and applies benchmark geography/ROI admission before heavy combine stages; the final analysis layer is the GT basic-only analysis package and uses documented NCCS/IRS classification enrichment where GT lacks NTEE fields.
- Datatype transformation: raw GT CSV files are preserved unchanged in Bronze S3; a normalized Combined cache parquet, admitted pre-Silver parquets, Silver benchmark parquets, and final analysis Parquet outputs are generated in later stages.
- Data cleaning: cleaning includes catalog-driven file selection, header-based dictionary extraction, low-risk field normalization, ROI admission, deterministic filing deduplication, benchmark geography assignment, Basic/Combined curation, calculated revenue and grant fields, classification enrichment, and imputed/proxy flags.
- Analysis data dictionary: use `docs/deliverables/preprocessing/technical_reference/variable_mappings/givingtuesday_basic_analysis_variable_mapping.md` plus `SWB_321_DATA_ROOT/raw/givingtuesday_990/datamarts/metadata/givingtuesday_990_basic_allforms_analysis_variable_coverage.csv`; raw dictionary support is under `documentation/final_preprocessing_docs/technical_docs/source_dictionaries/givingtuesday_datamarts/` (repository clone: `docs/deliverables/preprocessing/technical_reference/source_dictionaries/givingtuesday_datamarts/`).

## Script Order And Responsibilities

The current canonical GT pipeline lives in:

- `python/ingest/givingtuesday_990/datamart/`

The canonical orchestrator is:

- `python/ingest/givingtuesday_990/datamart/run_990_datamart_pipeline.py`
...
The per-file GT download URLs are catalog-derived and not hard-coded. They are selected from the catalog response, written into the flattened catalog and required-dataset manifests, and verified through source/local/S3 byte checks.

## Raw Source Dictionaries

- Datamart catalog Markdown: `documentation/final_preprocessing_docs/technical_docs/source_dictionaries/givingtuesday_datamarts/datamart_catalog.md`
- Datamart fields Markdown: `documentation/final_preprocessing_docs/technical_docs/source_dictionaries/givingtuesday_datamarts/datamart_fields.md`
- Datamart catalog CSV: `documentation/final_preprocessing_docs/technical_docs/source_dictionaries/givingtuesday_datamarts/datamart_catalog.csv`
- Datamart fields CSV: `documentation/final_preprocessing_docs/technical_docs/source_dictionaries/givingtuesday_datamarts/datamart_fields.csv`
- These artifacts are derived from the GT public catalog API and document the raw datamart files and source fields used by this pipeline.

The script:

- `python/ingest/givingtuesday_990/datamart/01_fetch_datamart_catalog.py`
...
- `python/ingest/givingtuesday_990/datamart/02_export_data_dictionary.py`
...
- `python/ingest/givingtuesday_990/datamart/03_download_raw_datamarts.py`
...
- Generated coverage CSV: `01_data/raw/givingtuesday_990/datamarts/metadata/givingtuesday_990_basic_allforms_analysis_variable_coverage.csv`
- Former published S3 documentation object (historical): `silver/givingtuesday_990/analysis/documentation/givingtuesday_datamart_pipeline.md`
- Former published S3 variable mapping object (historical): `silver/givingtuesday_990/analysis/variable_mappings/givingtuesday_basic_analysis_variable_mapping.md`
- Former published S3 coverage object (historical): `silver/givingtuesday_990/analysis/quality/coverage/givingtuesday_990_basic_allforms_analysis_variable_coverage.csv`
- Coverage CSVs are generated by the analysis step and published as authoritative quality evidence.
...
- `docs/deliverables/preprocessing/technical_reference/variable_mappings/`
...
- Silver analysis documentation:
  - `silver/givingtuesday_990/analysis/documentation/`
- Silver analysis variable mappings:
  - `silver/givingtuesday_990/analysis/variable_mappings/`
- Silver analysis coverage:
  - `silver/givingtuesday_990/analysis/quality/coverage/`
...
- `python/ingest/givingtuesday_990/datamart/06_build_basic_allforms_presilver.py`
...
- `python/ingest/givingtuesday_990/datamart/07_build_basic_plus_combined_presilver.py`
...
- `python/ingest/givingtuesday_990/datamart/08_filter_benchmark_outputs_local.py`
...
- `python/ingest/givingtuesday_990/datamart/13_extract_basic_analysis_variables_local.py`
...
- `givingtuesday_990_basic_allforms_analysis_variable_coverage.csv`
- `docs/deliverables/preprocessing/technical_reference/variable_mappings/givingtuesday_basic_analysis_variable_mapping.md`
...
```bash
python -m pytest tests/givingtuesday_990/test_datamart_pipeline.py -q
```
...
- step-14 upload to `silver/givingtuesday_990/analysis/documentation/`
- step-14 upload to `silver/givingtuesday_990/analysis/variable_mappings/`
- step-14 upload to `silver/givingtuesday_990/analysis/quality/coverage/`
...
| 14   | step-13 local analysis outputs                                                          | GT analysis S3 objects under analysis, documentation, variable-mapping, and coverage prefixes |
...
```bash
python python/ingest/givingtuesday_990/datamart/run_990_datamart_pipeline.py
```

### Run the final GT analysis extract

```bash
python python/ingest/givingtuesday_990/datamart/13_extract_basic_analysis_variables_local.py
```
