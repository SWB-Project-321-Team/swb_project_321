...
These files are included in the client-ready package. In this repository they live under `docs/deliverables/preprocessing/technical_reference/source_dictionaries/`; while the former bucket was active, published S3 keys used `documentation/final_preprocessing_docs/technical_docs/source_dictionaries/`. The bucket was intentionally deleted by 2026-07-23, so those keys are historical lineage only.
