...
and writes docs/data_sources/irs_990/sample_parsed_irs_990_10.csv.
...
Run from repo root:
  python scripts/diagnostics/irs_990/sample_parsed_990_to_csv.py
  AWS_PROFILE=neilken-admin python scripts/diagnostics/irs_990/sample_parsed_990_to_csv.py
"""
import argparse
import os
import re
...
PARSER_SCRIPT = REPO_ROOT / "python" / "ingest" / "irs_990" / "03_parse_irs_990_zips_to_staging.py"
...
def main() -> None:
    ...
    if use_all:
        ...
        sample_csv = REPO_ROOT / "docs" / "data_sources" / "irs_990" / "sample_parsed_irs_990_all.csv"
    else:
        n = max(1, args.num if args.num is not None else DEFAULT_SAMPLE_SIZE)
        sample_csv = REPO_ROOT / "docs" / "data_sources" / "irs_990" / f"sample_parsed_irs_990_{n}.csv"
