...
Run from repo root:
  python scripts/diagnostics/irs_990/fetch_sample_xml_and_test_parser.py
  python scripts/diagnostics/irs_990/fetch_sample_xml_and_test_parser.py --local-zip path/to/file.zip
  python scripts/diagnostics/irs_990/fetch_sample_xml_and_test_parser.py --year 2024 --count 10
"""
import os
import sys
import zipfile
...
# Repo root and python on path (this file lives in tests/irs_990/)
_FILE_DIR = Path(__file__).resolve().parent
_REPO_ROOT = Path(__file__).resolve().parents[3]
_PYTHON = _REPO_ROOT / "python"
_990_IRS_SRC = _REPO_ROOT / "python" / "ingest" / "irs_990"
if str(_PYTHON) not in sys.path:
    sys.path.insert(0, str(_PYTHON))
from utils.paths import DATA

# Import parser and S3 listing from 03 (source lives in python/ingest/irs_990/)
